...
metadata.json                    KPlugin: Id, Name, Icon, Category, License, Version
                                 + X-Plasma-NotificationAreaCategory для трея, см. 3
contents/
...
### Без ключа в метаданных трея не будет вообще

Прежде чем спорить о шапке, аплет должен попасть в список *Настроить область
уведомлений → Записи*. Решает это одна строка в `metadata.json` —
`applets/systemtray/plasmoidregistry.cpp` **[источник]**:
...
Кандидатом трей считает аплет с **непустым** `X-Plasma-NotificationAreaCategory`.
Ключи объявляются рядом с `KPlugin`, а не внутри него:

```json
{
    "KPackageStructure": "Plasma/Applet",
    "KPlugin": { "Id": "org.example.plasma.thing", "…": "…" },
...
Категория задаёт группу в списке записей: `ApplicationStatus`, `Communications`,
`SystemServices`, `Hardware`. `X-Plasma-NotificationArea` фильтром на master уже
не является, но его ставят все системные аплеты-пакеты (`org.kde.plasma.vault`,
`org.kde.kdeconnect`) — писать стоит оба.

Симптом отсутствия ключа коварен тем, что ни на что не похож: виджет ставится,
работает, добавляется в панель, но **в списке записей трея его нет**, и настроить
«показывать при необходимости» нечем. Ошибок при этом не печатается ни одной.

Проверка после установки:

```sh
python3 -c "import json;print(json.load(open('metadata.json')).get('X-Plasma-NotificationAreaCategory'))"
```

### Трей или панель — решается для каждого экземпляра

Ключ ничего не запрещает: аплет с ним по-прежнему добавляется прямо в панель или
на рабочий стол. Но **виджет, спроектированный для трея, в панели теряет часть
интерфейса**, и это надо понимать заранее:

| Что | В трее | В панели |
|---|---|---|
| `Plasmoid.status` | прячет и подсвечивает значок | не значит ничего, виджет виден всегда |
| Действия `HighPriority` | кнопки в шапке трея | шапки нет, действия только в контекстном меню |
| Размер попапа | задаёт трей, `gridUnit * 24` | по содержимому |

То есть автопрятанье «когда всё спокойно» и кнопки в шапке существуют **только**
в трее. Если виджет обязан быть виден постоянно — это режим «Всегда показывать»
у записи трея, а не размещение в панели.

Место размещения хранится в `~/.config/plasma-org.kde.plasma.desktop-appletsrc`;
найти, где сидит экземпляр, можно так:

```sh
grep -n -B40 'plugin=<id аплета>' ~/.config/plasma-org.kde.plasma.desktop-appletsrc \
    | grep -E '^[0-9]+[-:]\[|plugin='
```

Если контейнер экземпляра — не тот, у которого `plugin=org.kde.plasma.systemtray`,
виджет лежит в панели.
...
## 7.5. Состояние в JS: как уронить plasmashell

Это стоило четырёх падений оболочки, поэтому вынесено отдельно.

**Нельзя держать долгоживущий JS-объект с меняющимися ключами в `property var`,
и тем более хранить в нём функции.** Такой код валит plasmashell:

```qml
property var callbacks: ({})        // живёт всё время работы виджета

function run(cmd, cb) {
    callbacks[cmd] = cb;            // ключи появляются
}
onNewData: {
    var cb = callbacks[source];
    delete callbacks[source];       // и исчезают
    cb(...);                        // а ещё это функция
}
```

Симптом: `SIGSEGV` в `QV4::Object::insertMember`, вызванный из `internalPut` из
`Runtime::StoreElement` внутри обработчика сигнала. Проявляется «иногда» — когда
состояние меняется одновременно с приходом данных: при закрытии попапа, после
разблокировки экрана. Одинаковая подпись у нескольких дампов — верный признак,
что дело в этом, а не в самой Plasma. **[замер: 4 дампа]**

Как делать правильно:

- результат помечать **тегом-строкой** и разбирать его в одной функции-диспетчере;
- наружу отдавать **сигналом**, а не колбэком;
- в очереди хранить **свежие объекты фиксированной формы** (`{cmd, tag, timeout}`),
  которые создаются и выбрасываются целиком, а не мутируются годами;
- массивы в `property var` менять через локальную переменную с обратным
  присваиванием (`var q = queue; q.push(x); queue = q;`).

Проверить существующий виджет на эту ошибку:

```sh
# =[^=] отсекает сравнения === и ==, иначе половина совпадений ложная
grep -nE '\w+\[[^]]+\] *=[^=]|delete \w+\[' contents/ui/*.qml
```

## 7.6. Анимация активности

Приём из самого трея (`applets/systemtray/qml/PulseAnimation.qml`)
**[источник]**: короткий рост масштаба до 1.2 и обратно, затем длинная пауза,
цикл `Kirigami.Units.veryLongDuration * 5`, доли 15 % / 15 % / 70 %.
...
SequentialAnimation {
    ...
    alwaysRunToEnd: true
    onRunningChanged: if (!running) icon.scale = 1   // иначе останется 1.2

    ScaleAnimator { target: icon; from: 1;   to: 1.2; duration: pulse.cycle * 0.15 }
    ScaleAnimator { target: icon; from: 1.2; to: 1;   duration: pulse.cycle * 0.15 }
    PauseAnimation { duration: pulse.cycle * 0.7 }
}
```

Длинная пауза здесь принципиальна: непрерывное движение в панели раздражает, а
редкий «вздох» замечаешь, не отвлекаясь.

Если значок показывается только во время активности (`PassiveStatus` в покое),
добавьте **задержку скрытия** на несколько секунд. Без неё короткая операция даёт
мигание: значок появился и сразу пропал, заметить его не успеваешь. Заодно
остаётся время увидеть итог — «Синхронизировано» вместо мгновенного исчезновения.

## 8. Значки

- Только имена из темы (`cloudstatus`, `dialog-error`, `view-refresh`,
...
- Для панелей и трея предпочтительны symbolic-варианты (`go-previous-symbolic`),
  как в самом трее **[источник]**. Но суффикс `-symbolic` монохромности **не
  гарантирует**: из 7628 таких файлов в Breeze 454 применяют
  `class="ColorScheme-Accent"` **[замер]**. Значок трея выбирать по следующему
  разделу, а не по суффиксу.
- Одно имя может жить в разных категориях с разным набором размеров:
  `folder-symbolic` есть в `places/16,22,24` и в `actions/32` **[замер]**.
  Поэтому размеры искать по всей теме, а не в одном каталоге.

### Значок трея: один и тот же значок в разных размерах — разные файлы

В Breeze размер значка выбирает не масштабирование, а отдельный файл, и
**раскраска в этих файлах разная** **[замер]**:

```
places/22/folder-cloud.svg   class="ColorScheme-Text"     монохромный
places/32/folder-cloud.svg   class="ColorScheme-Accent"   синий
```

Значок в трее рисуется размером панели. На тонкой панели берётся вариант ≤24 и
выглядит монохромным, на толстой — вариант ≥32, и виджет оказывается
единственным цветным пятном в ряду. Симптом («иконка стала цветной, а я ничего
не менял») выглядит как что угодно, только не как выбор файла по размеру.

Правило: **для значка трея брать имена из категории `status`, у которых в теме
нет вариантов крупнее 24 px.** Такие есть только в 16/22/24 и потому монохромны
при любой высоте панели. Если нужного по смыслу значка среди них нет, годится
symbolic-вариант обычного имени — `folder-cloud-symbolic` существует только в
16/22/24 и покрашен `ColorScheme-Text`, тогда как у `folder-cloud` есть цветные
32–96 **[замер]**. Но проверять раскраску всё равно придётся: см. оговорку про
суффикс выше.

Проверенные имена **[замер]**:

| Имя | Размеры | Раскраска |
|---|---|---|
| `cloudstatus` | 16 22 24 | `ColorScheme-Text` |
| `media-playback-paused` | 16 22 24 | `ColorScheme-Text` |
| `network-offline` | 22 24 | `ColorScheme-Text` + `NegativeText` |
| `state-ok`, `state-error`, `state-pause`, `state-warning` | 16 22 24 | `Text` + семантический |
| `dialog-warning` | 16 22 24 64 | `ColorScheme-NeutralText` |

Цвет допустим только семантический и только из палитры темы
(`NeutralText`, `NegativeText`) — так окрашены значки сети и батареи. Хардкод в
файле значка (`fill:#3daee9` у `state-sync`, `fill:#bdc3c7` у `state-offline`)
темой не перекрашивается, такие имена для трея не годятся.

Проверка перед сдачей — заодно ловит **имена, которых в теме нет вообще**
(`cloud-offline` выглядит правдоподобно, но его не существует, и значок молча
подменяется запасным):

```sh
for n in cloudstatus network-offline dialog-warning; do
    printf '%-24s ' "$n"
    find /usr/share/icons/breeze -name "$n.svg" \
        | sed 's|.*/\([0-9]\+\)/.*|\1|' | sort -n | uniq | tr '\n' ' '
    echo
done
```

Пустой список размеров — имени в теме нет. Размеры от 32 и выше — смотреть, чем
этот файл покрашен.

Набор из таблицы выше собран для этого виджета целиком и подтверждён в живой
оболочке: в ряду с сетью, звуком и батареей значок больше не выделяется.

Для `KPlugin.Icon` в `metadata.json` правило обратное: он показывается в списке
«Добавить виджеты» крупно, поэтому там уместен именно цветной значок с большими
вариантами.
...
- [ ] для виджета трея в `metadata.json` задан `X-Plasma-NotificationAreaCategory`,
      и он виден в *Настроить область уведомлений → Записи*
...
- [ ] все имена значков существуют в теме, а у значка трея нет вариантов ≥32 px
...
| Файл | Что даёт |
|---|---|
| `applets/systemtray/plasmoidregistry.cpp` | по какому ключу метаданных аплет вообще попадает в трей |
